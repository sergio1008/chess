...
class Tablero {
  ...
  crearFichas = () => {
    for (let i = 0; i < 8; i++) {
      if (i == 0) {
        let t1 = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 0
        );
        this.tablero[t1].ficha = white_rook;
        this.tablero[t1].color = "white";
        let c1 = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 1
        );
        this.tablero[c1].ficha = white_knight;
        this.tablero[c1].color = "white";
        let a1 = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 2
        );
        this.tablero[a1].ficha = white_bishop;
        this.tablero[a1].color = "white";
        let q = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 3
        );
        this.tablero[q].ficha = white_queen;
        this.tablero[q].color = "white";
        let k = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 4
        );
        this.tablero[k].ficha = white_king;
        this.tablero[k].color = "white";
        let a2 = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 5
        );
        this.tablero[a2].ficha = white_bishop;
        this.tablero[a2].color = "white";
        let c2 = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 6
        );
        this.tablero[c2].ficha = white_knight;
        this.tablero[c2].color = "white";
        let t2 = this.tablero.findIndex(
          caj => caj.cordenada[0] == i && caj.cordenada[1] == 7
        );
      ...
      if (i == 1) {
        for (let e = 0; e < 8; e++) {
          let cj = this.tablero.findIndex(
            caj => caj.cordenada[0] == i && caj.cordenada[1] == e
          );
  ...
  jugada = () => {
    if (this.fichaSeleccionada) {
      let index = this.tablero.findIndex(
        tb => tb.cordenada == this.fichaActual.cordenada
      );
      let nueva = this.tablero.findIndex(
        tb => tb.cordenada == this.fichaFutura.cordenada
      );
